...
class Site
{
    ...
    private function getLinkNameByCurrentDir()
    {
        ...
        if ($count > 1) {
            throw new DomainException("There are {$count} links related to the current directory, please specify the name: laraserve unlink <name>.");
        }
    }
    ...
    /**
     * Pretty print out all links in Laraserve.
     *
    ...
    /**
     * Pretty print out all parked links in Laraserve
     *
     * @return \Illuminate\Support\Collection
     */
    function parked()
    ...
    function resecureForNewTld($oldTld, $tld)
    {
        ...
        foreach ($secured as $url) {
            ...
            if (strpos($siteConf, '# laraserve stub: proxy.laraserve.conf') === 0) {
                $this->unsecure($url);
    ...
    function createCa()
    {
        $caPemPath = $this->caPath('LaravelLaraserveCASelfSigned.pem');
        $caKeyPath = $this->caPath('LaravelLaraserveCASelfSigned.key');

        if ($this->files->exists($caKeyPath) && $this->files->exists($caPemPath)) {
            return;
        }

        $oName = 'Laravel Laraserve CA Self Signed Organization';
        $cName = 'Laravel Laraserve CA Self Signed CN';
        ...
        $this->cli->runAsUser(sprintf(
            ...
            $oName, $cName, 'rootcertificate@laravel.laraserve', $caKeyPath, $caPemPath
        ));
    ...
    function createCertificate($url)
    {
        $caPemPath = $this->caPath('LaravelLaraserveCASelfSigned.pem');
        $caKeyPath = $this->caPath('LaravelLaraserveCASelfSigned.key');
        $caSrlPath = $this->caPath('LaravelLaraserveCASelfSigned.srl');
    ...
    function createSigningRequest($url, $keyPath, $csrPath, $confPath)
    {
        $this->cli->runAsUser(sprintf(
            ...
            $keyPath, $csrPath, $url, $url, '@laravel.laraserve', $confPath
        ));
    }
    ...
    {
        $config = str_replace('LARASERVE_DOMAIN', $url, $this->files->get(__DIR__.'/../stubs/openssl.conf'));
    ...
    {
        if ($siteConf === null) {
            $siteConf = $this->files->get(__DIR__.'/../stubs/secure.laraserve.conf');
        }

        return str_replace(
            ['LARASERVE_HOME_PATH', 'LARASERVE_SERVER_PATH', 'LARASERVE_STATIC_PREFIX', 'LARASERVE_SITE', 'LARASERVE_CERT', 'LARASERVE_KEY'],
            [
                $this->laraserveHomePath(),
                LARASERVE_SERVER_PATH,
                LARASERVE_STATIC_PREFIX,
                $url,
    ...
    function unsecureAll()
    {
        ...
        if ($secured->count() === 0) {
            return info('No sites to unsecure. You may list all servable sites or links by running <comment>laraserve parked</comment> or <comment>laraserve links</comment>.');
        }
    ...
    function proxyCreate($url, $host)
    {
        ...
        $siteConf = $this->files->get(__DIR__.'/../stubs/proxy.laraserve.conf');

        $siteConf = str_replace(
            ['LARASERVE_HOME_PATH', 'LARASERVE_SERVER_PATH', 'LARASERVE_STATIC_PREFIX', 'LARASERVE_SITE', 'LARASERVE_PROXY_HOST'],
            [$this->laraserveHomePath(), LARASERVE_SERVER_PATH, LARASERVE_STATIC_PREFIX, $url, $host],
            $siteConf
        );

        $this->secure($url, $siteConf);

        info('Laraserve will now proxy [https://'.$url.'] traffic to ['.$host.'].');
    }
    ...
    function proxyDelete($url)
    {
        ...
        info('Laraserve will no longer proxy [https://'.$url.'].');
    }

    function laraserveHomePath()
    {
        return LARASERVE_HOME_PATH;
    }
    ...
    {
        return $this->laraserveHomePath().'/Nginx'.($additionalPath ? '/'.$additionalPath : '');
    }

    /**
     * Get the path to the linked Laraserve sites.
     *
     * @return string
     */
    function sitesPath($link = null)
    {
        return $this->laraserveHomePath().'/Sites'.($link ? '/'.$link : '');
    }

    /**
     * Get the path to the Laraserve CA certificates.
     *
     * @return string
     */
    function caPath($caFile = null)
    {
        return $this->laraserveHomePath().'/CA'.($caFile ? '/'.$caFile : '');
    }

    /**
     * Get the path to the Laraserve TLS certificates.
     *
     * @return string
     */
    function certificatesPath($url = null, $extension = null)
    {
        ...
        return $this->laraserveHomePath().'/Certificates'.$url.$extension;
    }
}
