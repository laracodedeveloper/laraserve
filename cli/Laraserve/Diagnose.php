...
class Diagnose
{
    var $commands = [
        'sw_vers',
        'laraserve --version',
        'cat ~/.config/laraserve/config.json',
        ...
        'curl --version',
        'php --ri curl',
        '~/.composer/vendor/laravel/laraserve/bin/ngrok version',
        'ls -al ~/.ngrok2',
        'brew info nginx',
        'brew info php',
        'brew info openssl',
    ...
    function run($print, $plainText)
    {
        ...
        $this->files->put('laraserve_diagnostics.txt', $output);

        $this->cli->run('pbcopy < laraserve_diagnostics.txt');

        $this->files->unlink('laraserve_diagnostics.txt');
