...
class PhpFpm
{
    ...
    /**
     * Forcefully uninstall all of Laraserve's supported PHP versions and configurations
     *
     * @return void
     */
    function uninstall()
    {
        ...
        rename('/usr/local/etc/php', '/usr/local/etc/php-laraserve-bak'.time());
    ...
    function updateConfiguration()
    {
        ...
        // rename (to disable) old FPM Pool configuration, regardless of whether it's a default config or one customized by an older Laraserve version
        ...
        if (false === strpos($fpmConfigFile, '5.6')) {
            // for PHP 7 we can simply drop in a Laraserve-specific fpm pool config, and not touch the default config
            $contents = $this->files->get(__DIR__.'/../stubs/etc-phpfpm-laraserve.conf');
            $contents = str_replace(['LARASERVE_USER', 'LARASERVE_HOME_PATH'], [user(), LARASERVE_HOME_PATH], $contents);
        } else {
            // for PHP 5 we must do a direct edit of the fpm pool config to switch it to Laraserve's needs
            $contents = $this->files->get($fpmConfigFile);
            $contents = preg_replace('/^user = .+$/m', 'user = '.user(), $contents);
            $contents = preg_replace('/^group = .+$/m', 'group = staff', $contents);
            $contents = preg_replace('/^listen = .+$/m', 'listen = '.LARASERVE_HOME_PATH.'/laraserve.sock', $contents);
    ...
    function fpmConfigPath()
    {
        ...
        return $versionNormalized === '5.6'
            ...
            : "/usr/local/etc/php/${versionNormalized}/php-fpm.d/laraserve-fpm.conf";
    }
    ...
    function validateRequestedVersion($version)
    {
        ...
        if (!$this->brew->supportedPhpVersions()->contains($version)) {
            throw new DomainException(
                sprintf(
                    'Laraserve doesn\'t support PHP version: %s (try something like \'php@7.3\' instead)',
                    $version
                )
            );
        }
