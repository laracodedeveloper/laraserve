...
class DnsMasq
{
    ...
    function install($tld = 'test')
    {
        ...
        // and then we put a Laraserve config file in there to point to the user's home .config/Laraserve/dnsmasq.d
        // This allows Laraserve to make changes to our own files without needing to modify the core dnsmasq configs
        ...
        info('Laraserve is configured to serve for TLD [.'.$tld.']');
    }
    ...
    function uninstall()
    {
        ...
        $this->cli->run('rm -rf /usr/local/etc/dnsmasq.d/dnsmasq-laraserve.conf');
    ...
    function ensureUsingDnsmasqDForConfigs()
    {
        ...
        // remove entries used by older Laraserve versions:
        $contents = preg_replace('/^conf-file.*laraserve.*$/m', '', $contents);

        // save the updated config file
        $this->files->put($this->dnsmasqMasterConfigFile, $contents);

        // remove old ~/.config/laraserve/dnsmasq.conf file because things are moved to the ~/.config/laraserve/dnsmasq.d/ folder now
        if (file_exists($file = dirname($this->dnsmasqUserConfigDir()) . '/dnsmasq.conf')) {
            unlink($file);
        }

        // add a Laraserve-specific config file to point to user's home directory Laraserve config
        $contents = $this->files->get(__DIR__.'/../stubs/etc-dnsmasq-laraserve.conf');
        $contents = str_replace('LARASERVE_HOME_PATH', LARASERVE_HOME_PATH, $contents);
        $this->files->ensureDirExists($this->dnsmasqSystemConfDir, user());
        $this->files->putAsUser($this->dnsmasqSystemConfDir . '/dnsmasq-laraserve.conf', $contents);

        $this->files->ensureDirExists(LARASERVE_HOME_PATH . '/dnsmasq.d', user());
    }
    ...
    {
        return $_SERVER['HOME'].'/.config/laraserve/dnsmasq.d/';
    }
}
